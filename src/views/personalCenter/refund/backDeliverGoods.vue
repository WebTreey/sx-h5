  ...
  <div class="back-deliver-goods">
    ...
    <div class="rfund-info">
      ...
      <div class="df mt-20">
        ...
        <a class="btn" @click="appDoBackDeliverGoodsFn">提交</a>
      </div>
    </div>
  </div>
</template>
<script>
import { appBackDeliverGoods,appDoBackDeliverGoods } from "api";
export default {
  data() {
    return {
  ...
  created() {
    this.appBackDeliverGoodsFn();
  },
  methods: {
    appBackDeliverGoodsFn() {
      appBackDeliverGoods({ backid: this.query.backid }).then(res => {
        console.log(res);
    ...
    appDoBackDeliverGoodsFn(){
        ...
        console.log(data)
        appDoBackDeliverGoods(data).then(res=>{
            this.$message('退货成功');
            this.$router.go(-1);
            console.log(res);
        })
